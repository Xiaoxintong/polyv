...
public final class StringUtil {
    ...
    private static Pattern VUE_PATH_PATTERN = Pattern.compile("^/v[0-9]*/");
    ...
    public static String trimAndDeleteSpecialEmoji(String input) {
        return trim(input).replaceAll("[\\uFE00-\\uFE0F]", "");
    }
     ...
     * @return
     */
    public static String strListToInSqlStringWithSingleQuotation(List<String> strList) {
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("(");
        for (int i = 0; i < strList.size(); i++) {
            String id = strList.get(i);
     ...
     * @return
     */
    public static boolean isXXTDomain(String urlStr) {
        boolean isXXTDomain = false;
        ...
        if (!isEmpty(urlStr)) {
            try {
                ...
                //image.xxt.cn/mobile/image/home_test.png 如果是这种没有协议的地址，
                // hostString 会直接返回null
                if (hostString.endsWith("xxt.cn")
                        || hostString.endsWith("xxt.net.cn")
                        || hostString.endsWith("hbjxt.cn")
                        || hostString.endsWith("lexue.cn")
                        || hostString.endsWith("xinzx.cn")) {
                    isXXTDomain = true;
                }
            } catch (Exception e) {
    ...
    public static boolean isXXTVueUrl(String urlStr) {
        boolean isXXTVueUrl = false;
        if (!isEmpty(urlStr)) {
            try {
                Uri uri = Uri.parse(urlStr);
                //针对以下域名，使用http的转https，没有协议的直接加htpps
                ...
                //image.xxt.cn/mobile/image/home_test.png 如果是这种没有协议的地址，
                // hostString 会直接返回null
                if (("m.xxt.cn".equals(hostString) || "hbjxt.cn".equals(hostString)
                        || "lexue.cn".equals(hostString)
                        || "xinzx.cn".equals(hostString))
                        && VUE_PATH_PATTERN.matcher(pathString).find()) {
                    isXXTVueUrl = true;
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    /**
     * 根据vue链接获取vue主页，因为有v2 v666这些，所以不能写死
     * @param urlStr
     * @return
     */
    public static String getXXTVueIndexUrl(String urlStr) {
        String indexUrl = urlStr;
        if (!isEmpty(urlStr)) {
            try {
                Uri uri = Uri.parse(urlStr);
                //针对以下域名，使用http的转https，没有协议的直接加htpps
                ...
                //image.xxt.cn/mobile/image/home_test.png 如果是这种没有协议的地址，
                // hostString 会直接返回null
                Matcher matcher = VUE_PATH_PATTERN.matcher(pathString);
                if (("m.xxt.cn".equals(hostString) || "hbjxt.cn".equals(hostString)
                        || "lexue.cn".equals(hostString)
                        || "xinzx.cn".equals(hostString))
                        && matcher.find()) {
                    ...
                    if (endIndex>0) {
                        ...
                        indexUrl= uri.getScheme()+ "://"+ hostString + portStr + pathString.substring(0, endIndex)+"index";
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
   ...
   public static boolean isYBTDomain(String urlStr) {
        boolean isYBTDomain = false;
        ...
        if (!isEmpty(urlStr)) {
            try {
                ...
                //image.xxt.cn/mobile/image/home_test.png 如果是这种没有协议的地址，
                // hostString 会直接返回null
                if (hostString.endsWith("youbeitong.cn")) {
                    isYBTDomain = true;
                }
            } catch (Exception e) {
    ...
    public static boolean isYXSDomain(String urlStr) {
        boolean isYXSDomain = false;
        ...
        if (!isEmpty(urlStr)) {
            try {
                ...
                if (hostString.endsWith("yunxiaosheng.com")) {
                    isYXSDomain = true;
                }
            } catch (Exception e) {
    ...
    public static boolean isXXTLoginDomain(String urlStr) {
        ...
        if (!isEmpty(urlStr)) {
            try {
                ...
                if ("login.xxt.cn".equals(hostString)) {
                    isXXTLoginDomain = true;
                }
            } catch (Exception e) {
    ...
    public static String getFormatPhoneNum(String phoneNum) {
        ...
        if (!StringUtil.isEmpty(phoneNum)) {
            targetPhoneNum = phoneNum.replaceAll(regex, "$1 $2 $3");
        }
    ...
    /**
     * 获取路径中最后的路径名 比如获取 /xxt/zhq/test 中的test
     * @param content
     * @return
     */
     ...
     * @return
     */
    public static String clearSpace(String targetStr) {
        String resultStr = "";
        if (!StringUtil.isEmpty(targetStr)) {
            resultStr = targetStr.replaceAll(" ", "");
        }
        return resultStr;
    }
